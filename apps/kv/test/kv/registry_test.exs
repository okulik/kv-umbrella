defmodule KV.RegistryTest do
  ...
  setup do
    ets = :ets.new(KV.Registry, [:set, :public])
  ...
  test "create registry by name", %{registry: registry, ets: ets} do
  ...
  test "removes the bucket on exit", %{registry: registry, ets: ets} do
    KV.Registry.create(registry, "shopping")
  ...
  test "removes the bucket on crash", %{registry: registry, ets: ets} do
    KV.Registry.create(registry, "shopping")
    {:ok, bucket} = KV.Registry.lookup(ets, "shopping")
  ...
  test "monitors existing bucket entries", %{registry: registry, ets: ets} do
    bucket = KV.Registry.create(registry, "shopping")
    
    # unlink registry to prevent shutting down the test itself
